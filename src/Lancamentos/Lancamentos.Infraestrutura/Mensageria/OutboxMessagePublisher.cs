...
public sealed class OutboxMessagePublisher : BackgroundService
{
    ...
    private readonly OutboxMessagePublisherOptions _opcoes;
    ...
    public OutboxMessagePublisher(
        ...
        IOptions<OutboxMessagePublisherOptions> opcoes,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _logger.LogInformation(
            "Publicador de outbox iniciado. IntervaloMs={IntervaloMs} Lote={Lote}",
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Falha inesperada no ciclo do publicador de outbox.");
            }
    ...
    private async Task PublicarPendentesAsync(CancellationToken cancellationToken)
    {
        ...
        var repositorio = scope.ServiceProvider.GetRequiredService<IOutboxMessageRepository>();
        ...
        foreach (var mensagem in mensagensPendentes)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Mensagem da outbox publicada. MensagemId={MensagemId} Tipo={Tipo} CorrelacaoId={CorrelacaoId}",
                    mensagem.Id,
                    mensagem.Tipo,
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogWarning(
                    ex,
                    "Falha ao publicar mensagem da outbox. MensagemId={MensagemId} Tipo={Tipo} CorrelacaoId={CorrelacaoId}",
                    mensagem.Id,
                    mensagem.Tipo,
                    mensagem.CorrelacaoId);
            }
