...
public sealed class MensagemSaidaTests
{
    [Fact]
    public void DeveCriarMensagemSaidaValida()
    {
        var ocorridaEmUtc = new DateTime(2026, 3, 17, 16, 0, 0, DateTimeKind.Utc);

        var mensagem = MensagemSaida.Criar(
            Guid.NewGuid(),
    ...
    public void DeveExigirOcorrenciaEmUtc()
    {
        ...
        var excecao = Assert.Throws<ExcecaoDominio>(() => MensagemSaida.Criar(
            Guid.NewGuid(),
            "LancamentoRegistradoV1",
            "{\"EventoId\":\"123\"}",
            "correlacao-123",
        ...
        Assert.Equal("A data de ocorrencia da mensagem de saida deve estar em UTC.", excecao.Message);
    }
}
