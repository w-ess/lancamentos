...
public sealed class ProcessarLancamentoRegistradoCasoDeUsoTests
{
    [Fact]
    public async Task DeveProcessarCreditoERegistrarLancamentoProcessado()
    {
        ...
        var casoDeUso = new ProcessarLancamentoRegistradoCasoDeUso(repositorio, relogio);

        var resultado = await casoDeUso.ExecutarAsync(CriarEvento("Credito", 125.50m));
    ...
    public async Task DeveIgnorarEventoDuplicadoPorLancamentoId()
    {
        ...
        var resultadoDuplicado = await casoDeUso.ExecutarAsync(evento with { EventoId = Guid.NewGuid() });
    ...
    public async Task DeveSomarDebitoEmSaldoExistente()
    {
        ...
        await casoDeUso.ExecutarAsync(CriarEvento("Credito", 200m, Guid.NewGuid()));
        await casoDeUso.ExecutarAsync(CriarEvento("Debito", 45m, Guid.NewGuid()));
    ...
    public async Task DevePropagarValidacaoDeDominioQuandoEventoForInvalido()
    {
        ...
        var excecao = await Assert.ThrowsAsync<ExcecaoDominio>(() =>
            casoDeUso.ExecutarAsync(CriarEvento("Transferencia", 30m)));
