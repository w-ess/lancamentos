...
public sealed class ConsultarSaldoDiarioPorDataCasoDeUsoTests
{
    [Fact]
    public async Task DeveRetornarSaldoExistenteSemDefasagemQuandoUltimoProcessamentoEstiverDentroDaTolerancia()
    {
        ...
        var resultado = await casoDeUso.ExecutarAsync(dataConsulta);
    ...
    public async Task DeveRetornarSaldoZeradoParaDataSemMovimentoUsandoUltimaConfirmacaoConhecida()
    {
        ...
        var resultado = await casoDeUso.ExecutarAsync(dataSemMovimento);
    ...
    public async Task DeveUsarHorarioAtualComoReferenciaQuandoAindaNaoHouverConfirmacaoConhecida()
    {
        ...
        var repositorio = new ConsolidadoDiarioRepositorioEmMemoria();
        var casoDeUso = CriarCasoDeUso(repositorio, agoraUtc, atrasoMaximoToleradoEmMinutos: 5);

        var resultado = await casoDeUso.ExecutarAsync(new DateOnly(2026, 3, 17));
    ...
    private static ConsultarSaldoDiarioPorDataCasoDeUso CriarCasoDeUso(
        ConsolidadoDiarioRepositorioEmMemoria repositorio,
        DateTime agoraUtc,
        int atrasoMaximoToleradoEmMinutos)
    {
        return new ConsultarSaldoDiarioPorDataCasoDeUso(
            repositorio,
